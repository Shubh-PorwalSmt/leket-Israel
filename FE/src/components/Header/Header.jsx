...
const Header = ({ CustomSearch }) => {
  ...
  const welcome =
    5 < currentHours && currentHours < 12
      ? "בוקר טוב אלירן"
      : 12 < currentHours && currentHours < 17
      ? "צהריים טובים אלירן"
      : "ערב טוב אלירן";
      ...
      >
        <Toolbar>
          <ButtonGroup variant="contained" sx={{ borderRadius: "10%" }}>
            <Button
              elevation={9}
              onClick={handleClickViewBtnColor}
              sx={leftButtonStyle}
            >
              <Box
                component="img"
                ...
                width={25}
                height={25}
                src="Images/header/gridForm.png"
                sx={{
                  filter:
                    leftBtnColor === "orange"
                      ? "brightness(0) invert(1)"
                      : "invert(34%) sepia(56%) saturate(372%) hue-rotate(83deg) brightness(75%) contrast(87%)",
                }}
              />
            </Button>
            <Button
