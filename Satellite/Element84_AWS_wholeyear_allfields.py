...
from shapely.geometry import Polygon # a bit about shapes: https://www.learndatasci.com/tutorials/geospatial-data-python-geopandas-shapely/
...
def plot_image(datatoplot,nameofdata,x_field,y_field,fieldcenter):
    fig=figure.Figure( figsize=(15, 10))
    axis = fig.subplots(
        ncols=1, nrows=1, subplot_kw={},squeeze =False
    )

    ax=axis[0,0]
    ax.imshow(datatoplot,extent=[min(x_field),max(x_field),min(y_field),max(y_field)]) 
...
def plot_index(datatoplot,nameofdata,x_field,y_field,fieldcenter):
    fig=figure.Figure( figsize=(15, 10))
    axis = fig.subplots(
        ncols=1, nrows=1, subplot_kw={},squeeze =False
    )
...
def satellite_index(data,bandlist,itemdate,**kwargs):
    ...
    if 'fieldpolygon_utm' in kwargs:
        x_field,y_field=([x[0] for x in kwargs['fieldpolygon_utm']],[x[1] for x in kwargs['fieldpolygon_utm']])

    else:
        x_field,y_field=(np.nan,),(np.nan,)
        fieldcenter=[np.nan,np.nan]
    if 'fieldcenter' in kwargs:
        fieldcenter=kwargs['fieldcenter']
    else:
        fieldcenter=[np.nan,np.nan]
...
## loading of a field with ID from shapefile:
...
field_idx=field_id_list.index(field_id)
fieldnames = [f[0] for f in field_shapes.fields[1:]]


# example for fieldname(4) and fieldID 206253
print(fieldnames[4]+': '+str(field_feature[field_id_list.index(field_id)].record[fieldnames[4]]))

print('polygon points: \n' + str(field_feature[field_id_list.index(field_id)].shape.points))

# converstion based on this reply: https://gis.stackexchange.com/questions/221797/python-pyproj-converting-from-wgs84-to-israel-grid-ig-05-12-epsg6991
itmproj=pyproj.Proj(init='epsg:2039')
utmproj=pyproj.Proj(init='epsg:4326')
fieldpolygon_deg=[pyproj.transform(itmproj, utmproj, x[0], x[1]) for x in field_feature[field_id_list.index(field_id)].shape.points] 
fieldpolygon = Polygon([p[0], p[1]] for p in fieldpolygon_deg)
...
search = client.search(
    ...
    bbox=bbox,
    datetime="2020-10-31/2022-12-31",
...
if dataset_small:
    ...
    for item in  items:
        ...
        utm_field_center = satellite_projection( point.x,point.y)
